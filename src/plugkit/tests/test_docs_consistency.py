...
def _current_docs() -> list[Path]:
    ...
    Nothing is exempt. The one-page build used to be a generated `.md` that had
    ...
    for pattern in ("docs/*.md", "docs/guide/*.md", "docs/design/*", "docs/steering/*"):
        files += sorted(REPO.glob(pattern))
    return [f for f in files if f.is_file() and f.suffix in (".md", ".md")]
...
def _rendered_docs() -> list[Path]:
    ...
    """
    files = [REPO / "docs/index.md"]
    for pattern in ("docs/guide/*.md", "docs/design/*.md", "docs/design/*.md",
                    "docs/steering/*.md"):
        files += sorted(REPO.glob(pattern))
...
def test_guide_snippets_import_what_they_name():
    ...
    incomplete = []
    for doc in sorted(REPO.glob("docs/guide/*.md")):
...
def test_api_reference_agrees_with_all():
    """A public name missing from the reference is a name nobody can look up.

    `docs/reference/index.md` is generated by quartodoc from the signatures and
    docstrings (`scripts/build-reference.py`), but *which* names it covers comes
    from the hand-written `quartodoc: sections:` block in `docs/_quarto.yml`. A
    name added to `__all__` and not to that block is documented nowhere, and
    nothing else notices.
    """
    page = (REPO / "docs/reference/index.md").read_text(encoding="utf-8")
    documented = {
        anchor.split(".", 1)[1]
        for anchor in re.findall(r"\{ #(plugkit\.[\w.]+) \}", page)
        if "." not in anchor.split(".", 1)[1]  # top-level names, not members
    }
    exports = _plugkit_exports()
...
def test_the_one_page_guide_holds_every_chapter():
    ...
    missing = []
    for chapter in sorted(REPO.glob("docs/guide/*.md")):
...
# ── I8: the reference can only cover what has a docstring ────────────────


def test_every_public_method_has_a_docstring():
    """quartodoc omits an undocumented member, so it vanishes from the reference.

    Not a style rule. `ctx.config.load_yaml` is taught in chapter 5 and was
    absent from the generated API reference for exactly this reason: no
    docstring, no entry, no error. A reader looking the method up found nothing
    and had no way to tell the page was incomplete.
    """
    import importlib
    import inspect

    plugkit = importlib.import_module("plugkit")
    undocumented = []
    for name in sorted(plugkit.__all__):
        exported = getattr(plugkit, name)
        if not inspect.isclass(exported):
            if not inspect.getdoc(exported):
                undocumented.append(name)
            continue
        for member, value in vars(exported).items():
            if member.startswith("_") or not callable(value):
                continue
            if not inspect.getdoc(value):
                undocumented.append(f"{name}.{member}")
    assert not undocumented, (
        "public API with no docstring, so absent from docs/reference/index.md:\n  "
        + "\n  ".join(undocumented)
    )
