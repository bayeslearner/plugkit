...
def test_api_reference_agrees_with_all():
    ...
    `docs/reference/index.qmd` is generated by quartodoc from the signatures and
    ...
    """
    page = (REPO / "docs/reference/index.qmd").read_text(encoding="utf-8")
    documented = {
        anchor.split(".", 1)[1]
...
def test_every_public_method_has_a_docstring():
    ...
    assert not undocumented, (
        "public API with no docstring, so absent from docs/reference/index.qmd:\n  "
        + "\n  ".join(undocumented)
    )
