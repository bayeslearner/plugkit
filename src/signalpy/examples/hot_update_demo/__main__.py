...
async def main():
    ...
    with tempfile.TemporaryDirectory(prefix="signalpy_plugins_") as tmpdir:
        ...
        result = await kernel.bus.invoke("plugin-loader.scan", {})
        ...
        for doc_id, text in [
            ...
            r = await kernel.bus.invoke("search.index_doc", {"id": doc_id, "text": text})
            print(f"    Indexed: {r}")
        ...
        print()
        loader_status = await kernel.bus.invoke("plugin-loader.status", {})
