...
from reactpy.kernel import Kernel, component, provides, requires, runnable, lifecycle
...
async def main():
    ...
    print()
    r = await kernel.bus.invoke("sensor.read", {"value": 100, "from_unit": "C", "to_unit": "F"})
    print(f"  {r['original']} → {r['converted']}")

    r = await kernel.bus.invoke("sensor.read", {"value": 72, "from_unit": "F", "to_unit": "C"})
    print(f"  {r['original']} → {r['converted']}")
