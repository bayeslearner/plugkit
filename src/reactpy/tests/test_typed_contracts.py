...
Verifies:
- Types accepted in @provides, @requires, @requires_aggregate, @requires_map, @requires_best
...
from reactpy.kernel import (
    ...
    requires_best, runnable, lifecycle, bind, unbind, prop,
)
from reactpy.kernel.component import get_meta, _finalize_meta, _contract_name, _is_contract_type
...
class TestTypedRequires:
    ...
    def test_requires_aggregate_with_type(self):
        ...
        @requires_aggregate("dicts", IDictionary)
        class C:
            pass
    ...
    def test_requires_best_with_type(self):
        @component("best-consumer")
        @requires_best("primary", IDictionary)
        class C:
            pass
...
class TestTypedSpellChecker:
    ...
    async def test_full_typed_flow(self):
        ...
        class Checker:
            ...
            @bind("dictionaries")
            def on_bind(self, svc, props):
                self.by_lang[props["language"]] = svc

            @unbind("dictionaries")
            def on_unbind(self, svc, props):
                self.by_lang.pop(props["language"], None)
