...
from reactpy.kernel import (
    Kernel, component, provides, requires, requires_aggregate,
    runnable, lifecycle, api, bind, unbind, prop,
)
...
class HttpSpellChecker:
    ...
    def on_bind(self, svc, props):
        self.by_lang[props.get("language", "?")] = svc

    @unbind("dicts")
    def on_unbind(self, svc, props):
        self.by_lang.pop(props.get("language", "?"), None)
