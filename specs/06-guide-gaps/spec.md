...
- [x] 1. Supervision chapter
  - [x] 1.1 Draft the chapter against `services/supervision.py`
    - **Requirements**: 1.1, 1.3
    - **Pillar**: Teaching
    - Added `docs/guide/08-supervision.qmd`; appended as chapter 08 so no
      renumbering was needed. Linked from chapter 1's `FAILED` discussion (R1.3).
  - [x] 1.2 Every example executed by `test_guide_examples.py`
    - **Depends**: 1.1
    - **Requirements**: 1.2
    - **Pillar**: Teaching, Test
    - Added `test_a_supervised_transient_failure_recovers` to
      `test_guide_examples.py`, executing the "Try it" block.
  - [x] 1.3 Renumber later chapters and fix every cross-reference
    - **Depends**: 1.1
    - **Pillar**: Documentation
    - Appended as 08, so 00-07 numbering and their cross-references were
      untouched. Cross-links to 08 added where the `FAILED` state is discussed.

- [x] 2. API reference
  - [x] 2.1 A reference page over the public surface
    - **Requirements**: 2.1
    - **Pillar**: Documentation
    - A generated page over `__all__` with one line of docstring each, wired
      into the site nav, sidebar and index. Added a docstring to `Fiber` and the
      tool decision classes. Superseded by `07-change-notification`'s successor,
      `08-api-reference`: that page was a name index, not a reference, and
      `docs/reference/index.qmd` replaced it.
  - [x] 2.2 A check that `__all__` and the reference agree
    - **Depends**: 2.1
    - **Requirements**: 2.2
    - **Pillar**: Documentation, Test
    - `test_api_reference_agrees_with_all` in `test_docs_consistency.py`.
...
**2026-08-24** — Implemented. Supervision chapter appended as 08 (no renumbering
needed — 00-07 stay put, contradicting the design note's assumption); API
reference page generated from `__all__` + docstrings, guarded by a new
`__all__`-vs-page test. Suite: 381 passed, 3 skipped, 2 xfailed. Closed SHIPPED.

**2026-08-24** — Superseded in part by `08-api-reference`. The page this spec
produced listed every public name and no signature; quartodoc now generates the
reference from the source, and the old page and its script are deleted.
