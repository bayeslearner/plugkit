...
| `await invoke(target, params?, *, timeout?)` | Call a handler. Optional timeout (raises `asyncio.TimeoutError`). Exact → target routing → name resolution → transports |
| `invoke_nowait(target, params?)` | Fire-and-forget. Schedules on event loop, returns immediately. Errors go to dead letter. |
...
# data = {"target": ..., "params": ..., "reason": ..., "error": ..., "timestamp": ...}
```

Reasons: `no_handler`, `handler_error`, `timeout`.
...
| `await self.rt.invoke(target, params?, *, timeout?)` | Bus call (policy-checked). Optional timeout raises `asyncio.TimeoutError`. |
