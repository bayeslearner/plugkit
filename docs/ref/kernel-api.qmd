...
Returns the `Effect` currently running, or `None` if not inside one.
Useful inside async effect bodies for cooperative supersede checks.
...
Returns `True` if a newer run has been scheduled while this body is
in flight. Always `False` for sync effects (sync bodies can't be
superseded mid-execution under the global lock). See
