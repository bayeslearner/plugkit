...
> `fn.__effect__ = EffectDef(...)`) and return the function unchanged.
> The kernel's `ComponentMeta` collects those markers when the class is
> built; the real wrapping (in `ReactiveEffect`, bus handler registration,
> etc.) happens at *activation* time. See
> [Reactivity by Example → Setup: how `@effect` becomes an `Effect` instance](../concepts/reactivity-by-example.qmd#setup-how-effect-becomes-an-effect-instance)
> for the three-stage walkthrough with the actual code.
