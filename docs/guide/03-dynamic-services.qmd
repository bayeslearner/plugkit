...
finished. We say the new run **supersedes** the old one — a fresh
notification cancels the relevance of an in-flight body.

The default is **always safe**: the engine remembers the new
notification and re-runs the effect once the in-flight body
(the one currently mid-`await`, not yet returned) completes. You
write the obvious code and it works:
...
For a recipe-shelf summary of all four escape hatches (the three above
plus cross-thread writes via `loop.call_soon_threadsafe`), with
worked-example scenarios you can lift into your own code, see
