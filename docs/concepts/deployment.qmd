...
The Bus supports pluggable `BusTransport`. When `invoke()` can't find a local handler, it tries registered transports.

`@exportable(transport="jsonrpc", discovery="mdns")` marks a component for remote export. A RemoteAdapter transport would read this metadata and register cross-process proxy handlers.
