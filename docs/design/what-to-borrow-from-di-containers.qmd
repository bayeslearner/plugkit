...
### 1. Async construction — this one is a defect, not a wish

`dependency-injector` has `Coroutine` and async `Resource`. plugkit has neither,
and the way it lacks them is the problem:
...
The service registers successfully and is a coroutine object. Every dependent
gets it, nothing raises, and the failure appears later as an attribute error on
something that looked constructed.

This is already half-solved next door: `_setup_teardown` refuses an async context
manager with a message naming the fix, precisely so it cannot fail quietly, and
the kernel already awaits an awaitable returned from a plugin's `apply`
(`src/plugkit/cordis/fiber.py`). What is missing is `provide()` awaiting its
factory. The smallest change on this page and the only one that closes a way to
be silently wrong.

### 2. `Selector` — the implementation named by a config value
...
### 3. `Factory` — promoted from example to shipped
