...
subtitle: "How signals.py works inside: five data structures, the tracking mechanism, three traced scenarios"
---

`src/plugkit/signals.py` is 597 lines and imports nothing from the kernel. It can
...
This is why `ConfigService` builds a new dict on every `set()` rather than
mutating in place — see `_bury` in `services/config.py`.
:::
...
An `async def` body is supported with one limitation: dependency tracking covers
only the signal reads that happen **before the first `await`**. Reads after an
await run in a different step, where `_active_consumer` has been reset. Read
every signal you need up front, then await with the captured values.

When a notification arrives while an async body is mid-await, the engine sets
`_pending_run` rather than dropping it, and the body runs again when it finishes.
Inside the body, `is_stale()` returns `True` once a newer run is queued, so a
long body can choose to stop early. `Effect(fn, cancel_on_supersede=True)` goes
further and cancels the in-flight task at supersede time, so the body sees
